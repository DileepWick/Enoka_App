  ...
  const handleConfirm = async () => {
    try {
      ...
      const response = await axios.put(
        `https://enokaback-7e8aa9803d2c.herokuapp.com/api/deliveryItems/updateStatusOfDeliveryItem/${deliveryItemId}`,
        { status }
      );
      ...
      if (response.status === 200) {
        ...
        if (itemType === "Gasket") {
          const response = await axios.put(
            `https://enokaback-7e8aa9803d2c.herokuapp.com/api/gaskets/increaseGasketQty/${itemId}`,
            { quantity }
          );
