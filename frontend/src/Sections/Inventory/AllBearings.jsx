...
import Delete_Bearing from "../../components/Inventory_Components/DeleteBearing.jsx";
...
const AllBearings = () => {
  ...
  useEffect(() => {
    ...
    // Show toast when bearing is deleted
    const bearingDeletedListener = () => {
      fetchBearings();
      toast.success("Bearing deleted successfully!");
    };

    // Register the stockUpdated event listener
    emitter.on("stockUpdated", fetchBearings);
    emitter.on("bearingDeleted", bearingDeletedListener);

    // Cleanup the event listener on component unmount
    return () => {
      emitter.off("stockUpdated", fetchBearings);
      emitter.off("bearingDeleted", bearingDeletedListener);
    };
  }, []);

  // Filter bearings based on branch and search term
  const filteredBearings = bearings.filter((bearing) => {
    const searchWords = searchTerm.trim().toLowerCase().split(/\s+/);
    const matchesBranch =
      !selectedBranch ||
      bearing.stock.some((stock) => stock.branch?.name === selectedBranch);

    // Ensure that all properties are defined before calling toLowerCase
    const matchesSearchTerm = searchWords.every((word) => {
      const partNumber = bearing.part_number ? bearing.part_number.toLowerCase() : "";
      const size = bearing.sizes ? bearing.sizes.toLowerCase() : "";
      const engine = bearing.engine?.engine_name
        ? bearing.engine.engine_name.toLowerCase()
        : "";
      const vendorName = bearing.vendor?.vendor_name
        ? bearing.vendor.vendor_name.toLowerCase()
        : "";
  ...
  const renderStockDetails = (bearing) => {
    if (selectedBranch) {
      const selectedStock = bearing.stock.find(
    ...
    <div className="container">
      {isLoading ? (
        ...
        <>
          ...
          <Table aria-label="Bearings inventory" className="mb-6">
            <TableHeader>
            ...
            <TableBody>
              {filteredBearings.map((bearing) => (
                <TableRow key={bearing._id} className="font-f1">
                  <TableCell>{bearing.part_number}</TableCell>
                  <TableCell>
                    {bearing.engine?.engine_name} {bearing.sizes}
                    <Chip
                      color="primary"
                      size="sm"
                      variant="bordered"
                  ...
                  <TableCell>{bearing.brand}</TableCell>
                  <TableCell>{renderStockDetails(bearing)}</TableCell>
                  <TableCell>
                    <User
                      avatarProps={{
                  ...
                  <TableCell>
                    <Delete_Bearing bearingId={bearing._id} />
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
