...
const CurrentDelivery = () => {
  ...
  const fetchDeliveryItems = async (deliveryId) => {
    try {
      const response = await axios.get(
        `http://localhost:3098/api/deliveryItems/getDeliveryItemsByDeliveryId/${deliveryId}`
      );
  ...
  const handleStartDelivery = async () => {
    ...
    try {
      await axios.put(
        `http://localhost:3098/api/delivery/${newDelivery._id}/status`,
        { status: "on delivery" }
      );
  ...
  const handleRemoveItem = async (itemId) => {
    try {
      ...
      await axios.delete(
        `http://localhost:3098/api/deliveryItems/deleteDeliveryItem/${itemId}`
      );
